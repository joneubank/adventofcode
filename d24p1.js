...
'use strict';
    ...
    function recursiveTest(a, b, unsorted, target, action)
    {
        var aSum = arraySum(a);
        ...
        if(aSum < target)
        {
            for (var i = 0; i < unsorted.length; i++)
            {
                  ...
                  )
                {
                    ...
                    recursiveTest(nextA, b, remainder, target, action);
                }
            ...
            if(bSum !== target)
            {
                for(var i = 0; i < unsorted.length; i++)
                {
                    ...
                    }else if(bSum + nextVal < target)
                    {
                        ...
                        var remainder = unsorted.slice(0,i).concat(unsorted.slice(i+1));
                        recursiveTest(a, nextB, remainder, target, action);
                    }
                }
            }
        }
    ...
    */
    ...
    rl.on('close', function()
    {
        ...
        var groupWeight = arraySum(packages) / 3;
