function solve (code) {
    ...
    for(var i = 0; i < instructions.length; i++) {
        parseInstructions(instructions[i], lightsOn);
    }

    return lightsOn.length;
}

function parseInstructions (instruction, lightsOn) {
    console.log("Running: " + instruction);
    var words = instruction.split(" "),
        start = words[words.length - 3].split(","),
        end = words[words.length - 1].split(",");

    switch(instruction.substring(0,7)) {
        case "turn on":
            turnOn(start, end, lightsOn);
            break;
        case "turn of":
            turnOff(start, end, lightsOn);
            break;
        default:
            toggle(start, end, lightsOn);
            break;
    }
...
function turnOn (start,end, lightsOn) {
    for(var i = Number(start[0]); i <= Number(end[0]); i++) {
        for(var ii = start[1]; ii <= end[1]; ii++) {
            var pos = i + "," + ii;
            if(lightsOn.indexOf(pos) < 0) {
                lightsOn.push(pos);
            }
        }
    }
...
function turnOff (start,end, lightsOn) {
    for(var i = Number(start[0]); i <= Number(end[0]); i++) {
        for(var ii = start[1]; ii <= end[1]; ii++) {
            var pos = i + "," + ii,
                index = lightsOn.indexOf(pos);
            if(index >= 0) {
                lightsOn.splice(index, 1);
            }
        }
    }
...
function toggle (start,end, lightsOn) {
    console.log("Toggle Range - X: " + start[0] + "," + end[0] + " || Y: " + start[1] + "," + end[1] + " || Lights On: " + lightsOn.length);
    for(var i = Number(start[0]); i <= Number(end[0]); i++) {
        for(var ii = Number(start[1]); ii <= Number(end[1]); ii++) {
            var pos = i + "," + ii;
            var index = lightsOn.indexOf(pos);
            // console.log("Toggle Position: " + pos + " || indexOf: " + index + " || lightsOn.length: " + lightsOn.length);
            ...
            console.log("Toggle Result for row: " + i + " - lightsOn.length: " + lightsOn.length);
    }
